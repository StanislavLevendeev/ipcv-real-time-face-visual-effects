...
class FaceWarping:
    ...
    def process_frame(self, frame):
        ...
        frame = self._add_beard_effect(
            frame, jaw_pts, lower_jaw_pts, chin_pt, face_width
        )
    ...
    def _apply_eyebrow_frown(self, frame, left_brow, right_brow, face_width):
        ...
        for src, dst in zip(
            np.vstack([left_brow, right_brow]), np.vstack([new_left, new_right])
        ):
            dx, dy = dst - src  # displacement vector
            dist_sq = (map_x - src[0]) ** 2 + (
                map_y - src[1]
            ) ** 2  # squared distance to landmark
    ...
    def _apply_jaw_warp(self, frame, lower_jaw_pts, chin_pt, face_width):
        ...
        chin_ext = int(
            face_width * self.CHIN_EXTENSION_RATIO
        )  # How much to extend the chin downward
        jaw_influence = (
            face_width * self.JAW_INFLUENCE_RATIO
        )  # Spread of jaw widening effect
        chin_influence = (
            face_width * self.CHIN_INFLUENCE_RATIO
        )  # Spread of chin extension effect
        roi_margin = int(
            face_width * self.JAW_ROI_MARGIN_RATIO
        )  # Extra padding around jaw/chin for smooth warping
    ...
    def _add_beard_effect(self, frame, jaw_pts, lower_jaw_pts, chin_pt, face_width):
        ...
        cheek_radius = int(
            face_width * self.BEARD_CHEEK_RADIUS_RATIO
        )  # Radius for cheek circles
        cheek_offset = int(
            face_width * self.BEARD_CHEEK_HEIGHT_OFFSET_RATIO
        )  # Vertical offset for cheeks
        chin_width = int(
            face_width * self.BEARD_CHIN_WIDTH_RATIO
        )  # Width of beard under chin
        chin_length = int(
            face_width * self.BEARD_CHIN_LENGTH_RATIO
        )  # Length of beard below chin
        blur_size = int(
            face_width * self.BEARD_BLUR_SIZE_RATIO
        ...
        gradient_start = int(
            face_width * self.BEARD_GRADIENT_START_RATIO
        )  # Where vertical gradient starts
        gradient_range = int(
            face_width * self.BEARD_GRADIENT_RANGE_RATIO
        )  # Range over which gradient fades
        ...
        for pt in jaw_pts[::2]:
            cheek_pt = (
                pt[0],
                max(0, pt[1] - cheek_offset),
            )  # Shift upward for cheek region
        ...
        beard = (frame * self.BEARD_DARKNESS).astype(
            np.uint8
        )  # Darken underlying pixels
        tint = np.full_like(frame, self.BEARD_TINT_BGR)  # Add subtle color tint
        beard = cv.addWeighted(
            beard, 0.8, tint, 0.2, 0
        )  # Blend darkened frame and tint

        # Combine beard with original frame using mask
        mask_3ch = np.stack([mask] * 3, axis=-1)  # Convert mask to 3 channels
        return ((frame * (1 - mask_3ch) + beard * mask_3ch)).astype(
            np.uint8
        )  # Blend overlay
    ...
    def _draw_debug_info(self, frame, jaw_pts, lower_jaw_pts, chin_pt):
        ...
        cv.polylines(
            frame, [cv.convexHull(lower_jaw_pts)], True, (255, 0, 0), 2
        )  # original jaw
        cv.polylines(
            frame, [cv.convexHull(new_jaw)], True, (0, 0, 255), 2
        )  # target jaw
